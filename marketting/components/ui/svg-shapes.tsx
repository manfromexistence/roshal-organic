...
/** Round a number to 4 decimal places to avoid hydration mismatches. */
...
export function SeedOfLifeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  const petals = Array.from({ length: 6 }, (_, i) => {
    ...
    return { x: R(cx + r * Math.cos(angle)), y: R(cy + r * Math.sin(angle)) };
  });
...
export function FlowerOfLifeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  const ring2a = Array.from({ length: 6 }, (_, i) => {
    const angle = (i * 60 * Math.PI) / 180;
    return {
      x: R(cx + 2 * r * Math.cos(angle)),
      y: R(cy + 2 * r * Math.sin(angle)),
    };
  });
  const ring2b = Array.from({ length: 6 }, (_, i) => {
    const angle = ((i * 60 + 30) * Math.PI) / 180;
    const d = R(r * Math.sqrt(3));
    return { x: R(cx + d * Math.cos(angle)), y: R(cy + d * Math.sin(angle)) };
  });
...
export function MetatronsCubeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  const ring1 = Array.from({ length: 6 }, (_, i) => {
    ...
    return {
      x: R(cx + ringR * Math.cos(angle)),
      y: R(cy + ringR * Math.sin(angle)),
    };
  });
  const ring2 = Array.from({ length: 6 }, (_, i) => {
    const angle = (i * 60 * Math.PI) / 180 - Math.PI / 2;
    return {
      x: R(cx + 2 * ringR * Math.cos(angle)),
      y: R(cy + 2 * ringR * Math.sin(angle)),
    };
  });
...
export function CubeWireshapeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  const outerVerts = Array.from({ length: 4 }, (_, i) => {
    ...
    return {
      x: R(cx + outer * Math.cos(angle)),
      y: R(cy + outer * Math.sin(angle)),
    };
  });
  const innerVerts = Array.from({ length: 4 }, (_, i) => {
    const angle = (i * 90 * Math.PI) / 180;
    return {
      x: R(cx + inner * Math.cos(angle)),
      y: R(cy + inner * Math.sin(angle)),
    };
  });
...
export function IcosahedronWireshapeSvg({
  ...
  const outer = Array.from({ length: 5 }, (_, i) => {
    ...
    return {
      x: R(cx + outerR * Math.cos(angle)),
      y: R(cy + outerR * Math.sin(angle)),
    };
  });
  const inner = Array.from({ length: 5 }, (_, i) => {
    const angle = ((i * 72 + 36) * Math.PI) / 180 - Math.PI / 2;
    return {
      x: R(cx + innerR * Math.cos(angle)),
      y: R(cy + innerR * Math.sin(angle)),
    };
  });
...
export function TorusTubeSvg({ className, flip = "none" }: ShapeSvgProps) {
  ...
  const circles = Array.from({ length: ringCount }, (_, i) => {
    ...
    const x = R(cx + majorR * Math.cos(angle));
    const y = R(cy + majorR * 0.35 * Math.sin(angle)); // foreshortened for 3/4 view
    // Scale radius for depth
    const scale = R(0.6 + 0.4 * Math.sin(angle));
    return { x, y, r: R(tubeR * scale), opacity: R(0.3 + 0.7 * scale) };
  });
