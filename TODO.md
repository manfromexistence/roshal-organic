...
## Last Updated
2026-04-27 - Collapsed storefront quick-category overflow into a first-row `More` dropdown, removed footer partner/payment scrollbars, made landing reveal animations replay on upward re-entry, deleted stale placeholder/internal cleanup files, tightened Catbox upload validation, kept the live storefront plus build pipeline locally verifiable, wired dashboard marketing links to live page ids, replaced raw CMS/product JSON editing with structured field inputs, hardened ImgBB uploads with clearer dashboard error handling, and connected the live marketing hero/content renderer to the dashboard-managed page hero fields without altering the storefront UI structure
