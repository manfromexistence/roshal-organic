    ...
    for (const cellKey of selectedCellsArray) {
      ...
      if (row) {
        ...
        if (cell) {
          ...
          const cellVariant = (cell.column.columnDef?.meta as any)?.cell
            ?.variant;
      ...
      try {
        ...
        for (
          ...
          for (
            ...
            const cellOpts = (column?.columnDef?.meta as any)?.cell;
            ...
            switch (cellVariant) {
              ...
              case "multi-select": {
                ...
                try {
                  ...
                  values = pastedValue
                    ? pastedValue.split(",").map((v: string) => v.trim())
                    : [];
                }

                const validated = values.filter((v) =>
                  matchSelectOption(v, options),
                ) as string[];
        ...
        if (updates.length > 0) {
          ...
          if (currentState.cutCells.size > 0) {
            ...
            for (const cellKey of currentState.cutCells) {
              const { rowIndex, columnId } = parseCellKey(String(cellKey));
              const column = columnById.get(columnId);
              const cellVariant = (column?.columnDef?.meta as any)?.cell
                ?.variant;
          ...
          if (rowVirtualizer) {
            ...
            if (newColumnId !== columnId) {
              requestAnimationFrame(() => {
                ...
                if (targetCellRetry) {
                  const scrollDirection = getScrollDirection(0, 0, direction);
      ...
      if (
        ...
        if (cellsToClear.length > 0) {
          ...
          for (const cellKey of cellsToClear) {
            const { rowIndex, columnId } = parseCellKey(String(cellKey));
            const column = columnById.get(columnId);
            const cellVariant = (column?.columnDef?.meta as any)?.cell?.variant;
            const emptyValue = getEmptyCellValue(cellVariant);
