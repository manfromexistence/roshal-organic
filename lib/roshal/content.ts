...
export async function getRoshalProducts() {
  try {
    const products = await db
      .select()
      .from(roshalProducts)
      .where(eq(roshalProducts.isPublished, true))
    ...
    const [existingProduct] = await db.select().from(roshalProducts).limit(1);
    return existingProduct ? [] : defaultRoshalProducts;
  } catch {
...
export async function getAllRoshalProducts() {
  try {
    ...
    return products.length ? products.map(mapProduct) : defaultRoshalProducts;
  } catch {
    return defaultRoshalProducts;
  }
}
